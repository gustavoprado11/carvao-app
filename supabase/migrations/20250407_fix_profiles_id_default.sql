-- Fix profiles.id to ensure it matches auth.uid()
-- Remove the default gen_random_uuid() to prevent id mismatch with auth.uid()

alter table public.profiles
  alter column id drop default;

-- Update the seed_profile function to ensure it always uses auth.uid() when creating new profiles
-- This ensures profile.id === auth.uid() for authenticated users
...
as $$
  ...
  values (
    coalesce(p_id, auth.uid()), -- Use auth.uid() if p_id is null
    lower(p_email),
    p_type,
